...
-- RLS Policies for Gate Events
ALTER TABLE public.gate_events ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Allow all for everyone" ON public.gate_events;
CREATE POLICY "Allow all for everyone" ON public.gate_events FOR ALL USING (true) WITH CHECK (true);
