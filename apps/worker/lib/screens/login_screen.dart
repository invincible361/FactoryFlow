  ...
  void _login() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        // Restrict to worker role only
        if ((employee.role ?? 'worker') != 'worker') {
          throw StateError(
            'This app is for Workers only. Your role is ${employee.role}. Please use the correct app.',
          );
        }

        // Restrict worker to mobile only (Android/iOS).
        final isMobile =
